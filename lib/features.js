...
function buildUniverse() {
  ...
  for (const s of b2) {
    ...
    Object.assign(r.raw, {
      ...
      rsLineNewHigh: s.rsLineNewHigh != null ? !!s.rsLineNewHigh : null,
      udVolRatio50: num(s.udVolRatio50),
    });
  }
  for (const s of cream)  {
    const r = ensure(s.ticker, s); if (!r) continue;
    r.screeners.push('creamy');
    r.raw.creamyScore = num(s.score);
    r.raw.earnMomScore = num(s.earnMomScore);   // earnings momentum layer (surprise/streak/revisions)
    r.raw.peadFlag = s.peadFlag != null ? !!s.peadFlag : null;
  }
  ...
  // Bulk/block deal BUY flag (docs/deals.json via lib/smartmoney — optional)
  try {
    const { loadDeals, hasRecentBulkBuy } = require('./smartmoney');
    const dealsData = loadDeals();
    if (dealsData) {
      for (const rec of map.values()) {
        try { rec.raw.bulkDealBuy = hasRecentBulkBuy(dealsData, rec.ticker, 30); }
        catch { rec.raw.bulkDealBuy = null; }
      }
    }
  } catch { /* optional */ }

  // Earnings acceleration (Screener.in quarters) — applied to every screener survivor,
  // not only Creamy. Distinct from earnMom (Yahoo PEAD surprise/revisions).
  try {
    const eq = loadJson('earnings-quality.json');
    if (eq) {
      const earnMap = require('./earnings-quality').buildMap(eq);
      for (const rec of map.values()) {
        const earn = earnMap.get(rec.ticker);
        if (!earn) continue;
        rec.raw.earnAccelScore = num(earn.score);
        rec.raw.earnInDrift = earn.inDrift != null ? !!earn.inDrift : null;
      }
    }
  } catch { /* optional */ }
...
function computeFeatures(rec) {
  ...
  f.rsLineNewHigh = x.rsLineNewHigh != null ? (x.rsLineNewHigh ? 1 : 0) : null;
  f.udVolRatio    = num(x.udVolRatio50);
  ...
  f.earnMom         = num(x.earnMomScore);
  // Acceleration score; small lift when still inside the post-results drift window.
  f.earnAccel       = (() => {
    const s = num(x.earnAccelScore);
    if (s == null) return null;
    return x.earnInDrift && s >= 50 ? Math.min(100, s + 10) : s;
  })();
  f.bulkDealBuy     = x.bulkDealBuy != null ? (x.bulkDealBuy ? 1 : 0) : null;
