...
//               | 'creamy' | 'bestpicks' | 'sniper' | 'trendingvalue',
//     signalType: e.g. 'BREAKOUT_VALID', 'APEX_BUY', 'DEBATE_HOT', 'CONFLUENCE_USS', 'TV_PICK',
//     ticker: string,
//     name: string|null,
//     sector: string|null,
//     entry: number|null,
...
function withLock(fn) {
  ...
  while (!acquired) {
    try {
      ...
      // Hard ceiling FIRST: no matter why we can't get the lock (stale holder,
      // or an OS that won't let us unlink it), never spin past LOCK_WAIT_MS —
      // proceed without the lock rather than wedge the generator forever.
      // (Better to risk a rare lost row than to hang market-refresh.sh.)
      if (Date.now() - start > LOCK_WAIT_MS) {
        console.warn('outcomes: lock wait exceeded, proceeding without lock');
        break;
      }
      // Stale lock (holder crashed without cleaning up) — try to break it.
      try {
        const st = fs.statSync(LOCK_PATH);
        if (Date.now() - st.mtimeMs > LOCK_STALE_MS) {
          try { fs.unlinkSync(LOCK_PATH); continue; }
          catch { /* can't unlink (locked by OS) — fall through to backoff + wait-ceiling */ }
        }
