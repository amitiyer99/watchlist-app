...
(function(){
  ...
  // Flatten the on-screen data panel into prompt facts. The panel used to be display-only,
  // so the model never saw ₹3,419 for INDOTECH and invented ~₹115 instead.
  function factsFromPanel(panelJson){
    if(!panelJson)return '';
    var secs;try{secs=JSON.parse(panelJson);}catch(e){return '';}
    if(!secs||!secs.length)return '';
    var lines=[];
    secs.forEach(function(sec){
      if(sec.t)lines.push(sec.t+':');
      (sec.m||[]).forEach(function(m){
        var row=(m[0]?m[0]+': ':'')+m[1]+(m[2]?' ('+m[2]+')':'');
        if(row.trim())lines.push('- '+row);
      });
      (sec.s||[]).forEach(function(g){ if(g[2])lines.push('- '+g[2]); });
    });
    return lines.join('\\n');
  }
  var _drGen=0;
  var _drCache={};
  async function doResearch(ticker,name,prompt,panelJson,price,force){
    overlay.classList.add('open');
    titleEl.textContent='\uD83E\uDDE0 '+(name||ticker);
    var panelHtml=window.drRenderPanel(panelJson);
    var aiHead=window.drAiHeading;
    var pxNum=Number(price);
    var pxStr=(isFinite(pxNum)&&pxNum>0)?('\\u20b9'+pxNum.toLocaleString('en-IN',{maximumFractionDigits:2})):'';
    var facts=factsFromPanel(panelJson);
    var cacheKey=[ticker,pxStr,prompt||'',panelJson||'',curProv].join('|');
    if(!force&&_drCache[cacheKey]){
      content.innerHTML=panelHtml+aiHead
        +'<div style="font-size:.68rem;opacity:.65;margin-bottom:8px">Same facts as last run \\u2014 showing cached analysis. <a href="#" id="dr-rerun" style="color:var(--ac,#00d4aa)">Re-run</a></div>'
        +fmtAi(_drCache[cacheKey]);
      var rerun=document.getElementById('dr-rerun');
      if(rerun)rerun.onclick=function(ev){ev.preventDefault();doResearch(ticker,name,prompt,panelJson,price,true);};
      return;
    }
    ...
    var systemPrompt='You are a sharp NSE India equity analyst. Be specific and quantitative, never generic. '
      +'CRITICAL: Use ONLY the prices, levels, scores and facts supplied in the user message. '
      +'Never invent a different spot price, EMA, RSI, MACD, support, resistance or target. '
      +'If a number is not provided, write "not in data" — do not guess. '
      +'Always reply in the exact markdown structure the user specifies — bold section headings and short bullets.';
    var base=prompt||((name||ticker)+' ('+ticker+') on NSE India. Should I buy, hold, or pass? Cover the setup, the bull case, key risks and a verdict for the next 1-2 weeks.');
    var grounded='STOCK: '+(name||ticker)+' ('+ticker+') | NSE India'
      +(pxStr?' | Last price: '+pxStr:'')
      +'\\n\\nFACTS FROM THIS PAGE (authoritative — quote these numbers, do not replace them):\\n'
      +(facts||('- Last price: '+(pxStr||'unknown')))
      +'\\n\\nQUESTION / CONTEXT:\\n'+base;
    var userPrompt=grounded+FORMAT_SPEC;
    ...
    var gen=++_drGen;
    try{
      var r=await window.DR_COMPLETE({provId:curProv,key:key,system:systemPrompt,prompt:userPrompt,onStatus:function(m){
        if(gen!==_drGen)return;
        content.innerHTML=panelHtml+aiHead+'<div class="dr-loading">'+String(m)+'\\u2026</div>';
      }});
      if(gen!==_drGen)return;
      var shown=r.text;
      if(r.switchedFrom)shown='_Switched to '+r.model+' \\u2014 '+r.switchedFrom+' did not return a usable answer._\\n\\n'+shown;
      _drCache[cacheKey]=shown;
      content.innerHTML=panelHtml+aiHead+fmtAi(shown);
    }catch(e){
      if(gen!==_drGen)return;
      var err=e&&e.message?e.message:String(e);
      var mdl='';try{mdl=localStorage.getItem('dr_model_'+curProv)||prov.model;}catch(x){mdl=prov.model;}
      content.innerHTML=panelHtml+aiHead+'<div style="color:#ef4444;padding:16px">'+err
        +'<div style="color:#8888a0;font-size:.8rem;margin-top:8px">Model: '+mdl+' \\u00b7 provider: '+prov.label
  ...
  document.addEventListener('click',function(e){
    ...
    doResearch(btn.dataset.rTicker||'',btn.dataset.rName||btn.dataset.rTicker||'',btn.dataset.rPrompt||'',btn.dataset.rPanel||'',btn.dataset.rPrice||'');
  });
})();
`;
