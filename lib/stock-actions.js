...
// ─── AI Research (shared) ───
(function(){
  // Render the model's markdown-ish reply the way the Multibagger page does — bold
  // section headings, real paragraphs, bullets — instead of one undifferentiated
  // pre-wrap block. Every page that uses lib/stock-actions (Marquee, Confluence,
  // FII/DII, Sniper, Triggers, Best Picks, Sectors, India Research) now formats
  // identically. HTML is escaped BEFORE markdown is applied, so model output can't
  // inject markup — the per-page copies never did that.
  function fmtAi(raw){
    var s=String(raw==null?'':raw)
      .replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    // headings: **Bold line**  -> accented block heading (uses the page's own accent)
    s=s.replace(/\\*\\*([^*]+)\\*\\*/g,'<strong style="color:var(--ac,#00d4aa);display:block;margin-top:12px;margin-bottom:4px">$1</strong>');
    // bullets: leading -, * or • -> indented list rows
    s=s.replace(/(?:^|\\n)[ \\t]*[-*\\u2022][ \\t]+([^\\n]+)/g,'<div style="margin:2px 0 2px 14px;text-indent:-10px">&bull; $1</div>');
    // paragraphs + line breaks
    s=s.replace(/\\n\\n/g,'</p><p style="margin:4px 0">').replace(/\\n/g,'<br>');
    return '<div style="line-height:1.7"><p style="margin:0">'+s+'</p></div>';
  }
