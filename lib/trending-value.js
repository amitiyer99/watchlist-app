...
const RULES = {
  ...
  maxPerIndustry: 3,     // see the cap below — 0 disables it
  maxPerSectorFallback: 6, // looser limit for names that only have a coarse sector label:
                           //   "Industrials" spans hundreds of unrelated businesses, so
                           //   treating it like "Sugar" would flag diversification as
                           //   concentration. 3 unrelated engineering firms are not one bet.
};

// ── The P/CF coverage gate ────────────────────────────────────────────────────
// P/CF comes from Yahoo one stock at a time (Tickertape does not expose it), so
// coverage builds up over days rather than arriving complete. That creates a trap
// worth being explicit about:
//
//   Deciles are ranked ACROSS THE UNIVERSE. If only 40% of stocks have a P/CF, the
//   decile a stock lands in is its rank among the 40% we happened to fetch — not
//   among the market. Every score would look like a real sixth factor while being an
//   artefact of fetch order. A partially-covered factor is worse than a missing one,
//   because a missing one is visible.
//
// So P/CF is scored only once coverage clears this threshold. Below it the factor is
// dropped market-wide and the composite renormalises over five, exactly as it does
// today — and the page reports the coverage climbing rather than quietly pretending.
const PCF_MIN_COVERAGE = 0.80;

// Yahoo reports operating cash flow in ABSOLUTE rupees; Tickertape reports market cap
// in ₹ crore. One crore = 1e7. Getting this backwards yields a P/CF wrong by seven
// orders of magnitude that still RANKS correctly — so the deciles would look sane
// while every displayed multiple was nonsense. Hence one function, unit-tested.
const CRORE = 1e7;
function pcfFrom(marketCapCr, operatingCashflowAbs) {
  if (marketCapCr == null || !isFinite(marketCapCr)) return null;
  if (operatingCashflowAbs == null || !isFinite(operatingCashflowAbs) || operatingCashflowAbs === 0) return null;
  return (marketCapCr * CRORE) / operatingCashflowAbs;
}

// ── The industry cap ──────────────────────────────────────────────────────────
// Cyclicals get cheap together. The first live run of this screen returned SEVEN
// sugar mills in 25 names, plus ten Materials — a "diversified basket" that was
// really one bet on the sugar cycle placed seven times. O'Shaughnessy's rules are
// silent on this because the published tests hold 25 names out of a US universe
// large enough that clustering is rarer; on ~1,600 Indian names it is the norm.
//
// A SECTOR cap alone does not fix it: Tickertape files Avadh/Dalmia/Dhampur/Uttam
// Sugar under Consumer Staples but Andhra/Magadh Sugar under Materials, so a cap of
// 3 per sector still admits six sugar mills. Hence grouping by industry — the
// data source's own industry field when it resolves, and a keyword classifier on
// the company name as the fallback, which is what actually catches the clusters.
const INDUSTRY_KEYWORDS = [
  [/\bsugar\b|sugars|\bsakthi\b|bio.?organic/i, 'Sugar'],
  [/textile|spinning|\byarn\b|denim|fabric|weav/i, 'Textiles'],
  [/\bpaper\b|paperboard|newsprint/i, 'Paper'],
  [/cement|\bclinker\b/i, 'Cement'],
  [/\bsteel\b|\biron\b|ferro|sponge|\btmt\b/i, 'Steel'],
  [/chemical|petrochem|organics|fertiliser|fertilizer|\bagro.?chem/i, 'Chemicals'],
  [/\bpetro\b|refiner|\boil\b|\bgas\b|\bcoal\b|energy/i, 'Oil, gas & energy'],
  [/\bbank\b|finserv|financ|\bnbfc\b|capital|housing fin|insur/i, 'Financials'],
  [/pharma|\bdrug\b|laborator|life ?scien|health/i, 'Pharma & healthcare'],
  [/\bauto\b|motor|\btyre\b|\bt[iy]res\b|axle|forg/i, 'Auto & components'],
  [/software|\binfotech\b|\bit\b services|technolog|\bdigital\b/i, 'Technology'],
  [/\bpower\b|electric|transmission|\bgrid\b/i, 'Power & utilities'],
  [/\bplast\b|polymer|packag|\bfilms\b|\bpipes?\b/i, 'Plastics & packaging'],
  [/construct|infra|realt|\bestate\b|builder|\bhousing\b/i, 'Construction & realty'],
  [/\btea\b|\bcoffee\b|\bfood\b|\bdairy\b|beverag|edible/i, 'Food & beverages'],
  [/ceramic|\btiles\b|sanitary|glass/i, 'Ceramics & glass'],
  [/\bmining\b|mineral|\bzinc\b|\bcopper\b|alumin/i, 'Metals & mining'],
];

// Prefer the source's own industry label; fall back to the name classifier; fall
// back again to sector. Sector-only names are grouped as "sector: X" so they are
// never silently merged with a real industry group.
function industryGroupOf(s) {
  if (s.industry && String(s.industry).trim()) return String(s.industry).trim();
  const hay = `${s.name || ''} ${s.ticker || ''}`;
  for (const [re, label] of INDUSTRY_KEYWORDS) if (re.test(hay)) return label;
  return s.sector ? `sector: ${s.sector}` : 'unclassified';
}
...
function runScreen(stocks, opts = {}) {
  ...
  // P/CF is skipped entirely when its coverage is too thin to rank fairly (see
  // PCF_MIN_COVERAGE). Skipping is done by treating the metric as absent for every
  // stock, so the existing renormalisation handles it — no special case downstream.
  const pcfCov = opts.pcfCoverage == null ? null : opts.pcfCoverage;
  const pcfGated = pcfCov != null && pcfCov < (opts.pcfMinCoverage ?? PCF_MIN_COVERAGE);
  const activeMetrics = METRICS.filter(m => !(m.id === 'pcf' && (pcfGated || pcfCov == null)));

  const perMetric = {};
  for (const m of METRICS) {
    perMetric[m.id] = activeMetrics.includes(m)
      ? scoreMetric(universe, m)
      : { byKey: new Map(), pctByKey: new Map(), rankedCount: 0, worstCount: 0, missingCount: universe.length };
  }
  ...
  // 6 — take the top N in momentum order, capped per industry. Names the cap
  // pushes out are kept in `displaced` and reported, not silently dropped.
  const chosen = [];
  const displaced = [];
  const perGroup = new Map();
  const limitFor = g => {
    if (!R.maxPerIndustry) return Infinity;
    // Coarse sector fallbacks get the looser limit — see maxPerSectorFallback.
    return /^sector: /.test(g) ? (R.maxPerSectorFallback || R.maxPerIndustry) : R.maxPerIndustry;
  };
  for (const r of rankable) {
    if (chosen.length >= R.topN) break;
    const g = industryGroupOf(r);
    const n = perGroup.get(g) || 0;
    if (n >= limitFor(g)) { displaced.push({ ...r, industryGroup: g }); continue; }
    perGroup.set(g, n + 1);
    chosen.push({ ...r, industryGroup: g });
  }
  // If the cap leaves the list short — a cheap decile genuinely dominated by two
  // or three industries — fill the remainder from the displaced names in momentum
  // order and MARK them, rather than shipping a list of 14 or pretending to a
  // diversification the data cannot support.
  const overCap = [];
  for (const r of displaced) {
    if (chosen.length >= R.topN) break;
    chosen.push({ ...r, overCap: true });
    overCap.push(r.ticker);
  }
  const picks = chosen.map((r, i) => ({ ...r, rank: i + 1 }));

  // Mix of the FINAL list, over-cap names included. Reporting only the within-cap
  // counts would understate the concentration actually shipped — the exact thing
  // the cap exists to make visible.
  const industryMix = (() => {
    const m = new Map();
    for (const r of chosen) m.set(r.industryGroup, (m.get(r.industryGroup) || 0) + 1);
    return [...m.entries()].sort((a, b) => b[1] - a[1]).map(([group, n]) => ({ group, n }));
  })();
  // What the list would have looked like with no cap — so the page can show the
  // concentration the cap actually prevented.
  const uncappedMix = (() => {
    const m = new Map();
    for (const r of rankable.slice(0, R.topN)) {
      const g = industryGroupOf(r);
      m.set(g, (m.get(g) || 0) + 1);
    }
    return [...m.entries()].sort((a, b) => b[1] - a[1]).map(([group, n]) => ({ group, n }));
  ...
  return {
    ...
    coverage: METRICS.map(m => ({
      ...
      gated: !activeMetrics.includes(m),
    })),
    pcf: {
      coverage: pcfCov,
      gate: opts.pcfMinCoverage ?? PCF_MIN_COVERAGE,
      gated: pcfGated || pcfCov == null,
      reason: pcfCov == null ? 'no cash-flow cache yet'
        : pcfGated ? `coverage ${(pcfCov * 100).toFixed(1)}% is below the ${((opts.pcfMinCoverage ?? PCF_MIN_COVERAGE) * 100).toFixed(0)}% gate`
        : 'scored',
    },
...
module.exports = {
  RULES, METRICS, runScreen, compositeOf, decilesFor, scoreMetric, isFinancial,
  industryGroupOf, INDUSTRY_KEYWORDS, PCF_MIN_COVERAGE, pcfFrom, CRORE,
};
