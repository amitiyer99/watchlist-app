...
// Empirical calibration of conviction -> P(beat Nifty 20d).
//
// Gate: needs >=50 matured points AND >=15 distinct entry dates (rows from the
// same day share one market path — they are not independent evidence).
// Method: decile bins + ISOTONIC (pool-adjacent-violators) smoothing so the
// probability can never DECREASE as conviction rises — raw bin rates on small
// samples routinely produce that embarrassment.
// The returned function carries `.calibrated` so UIs can label prior-based
// outputs as "uncalibrated" instead of presenting false precision.
// One row per ticker-EPISODE. A stock that stays on the list for 15 sessions produced
// ONE market outcome, not 15: without this, overlapping 20-day windows are counted as
// independent evidence (measured: 2,466 rows collapse to 609 episodes, so every ticker
// was being counted about 4x) and the entry-date gate is trivially satisfied by
// re-emissions rather than by genuinely new days.
function dedupeCalibrationEpisodes(rows) {
  ...
  for (const list of byTicker.values()) {
    ...
    for (const r of list) {
      ...
      if (nextOk == null || d >= nextOk) {
        out.push(r);
        // next independent episode starts once this 20-day window has fully elapsed
        const n = new Date(d); let c = 0;
        while (c < 20) { n.setDate(n.getDate() + 1); if (n.getDay() >= 1 && n.getDay() <= 5) c++; }
        nextOk = n;
      }
    }
  }
...
function buildCalibrator(outcomes) {
  ...
  const usable = rows.filter(r => {
    if (r.screener !== 'bestpicks' || r.score == null) return false;
    const res = r.results && r.results['20d'];
    if (!res || res.beatNifty == null) return false;
    // Censored rows are IMPUTED survivorship placeholders (a synthetic -25% with
    // beatNifty=false), not observed outcomes. Fitting a probability model on them
    // teaches it that missing price data is a loss.
    if (res.censored) return false;
    return true;
  });
  const pts = [];
  const dates = new Set();
  for (const r of dedupeCalibrationEpisodes(usable)) {
    pts.push({ conv: r.score, beat: r.results['20d'].beatNifty ? 1 : 0 });
    if (r.date) dates.add(r.date);
  }
  if (pts.length < 50 || dates.size < 15) {
    // NOT a probability. A linear ramp off the conviction score, standing in until there
    // is enough independent history to fit anything. It read 53-66% while the realised
    // beat rate was 41-45%, and the page presented it as "isotonic regression on realized
    // outcomes" — hence `calibrated:false`, which consumers MUST surface to the user.
    const prior = (conv) => clamp(0.5 + (conv - 50) / 250, 0.3, 0.82);
    prior.calibrated = false;
    prior.basis = { n: pts.length, entryDates: dates.size, reason: pts.length < 50
      ? `only ${pts.length} independent episodes (need 50)`
      : `only ${dates.size} distinct entry dates (need 15)` };
    return prior;
  }
  // decile bins
  ...
  for (let i = 0; i < bins; i++) {
    ...
    edges.push({ lo, hi, rate, n: slice.length });
  }
  // Isotonic regression via pool-adjacent-violators: merge any bin whose rate is
  // below its predecessor's until the sequence is monotone non-decreasing.
  const pooled = [];
  for (const e of edges) {
    pooled.push({ ...e });
    while (pooled.length > 1) {
      const a = pooled[pooled.length - 2], b = pooled[pooled.length - 1];
      if (b.rate >= a.rate) break;
      const n = a.n + b.n;
      pooled[pooled.length - 2] = { lo: a.lo, hi: b.hi, rate: (a.rate * a.n + b.rate * b.n) / n, n };
      pooled.pop();
    }
  }
  const fn = (conv) => {
    for (const e of pooled) if (conv >= e.lo && conv <= e.hi) return clamp(e.rate, 0.15, 0.9);
    // outside the observed conviction range: clamp to nearest pooled bin, don't
    // silently fall back to the prior (which caused mid-range discontinuities)
    if (pooled.length) {
      if (conv < pooled[0].lo) return clamp(pooled[0].rate, 0.15, 0.9);
      if (conv > pooled[pooled.length - 1].hi) return clamp(pooled[pooled.length - 1].rate, 0.15, 0.9);
    }
    return clamp(0.5 + (conv - 50) / 250, 0.3, 0.82);
  };
  fn.calibrated = true;
  fn.basis = { n: pts.length, entryDates: dates.size, bins: pooled.length };
  return fn;
}
...
function scoreUniverse(stocks, { regime, macro, outcomes } = {}) {
  ...
  for (const rec of stocks) {
    ...
    rec.winProbByHorizon = {};
    for (const h of Object.keys(HORIZON)) {
      rec.winProbByHorizon[h] = +calibrate(conv[h]).toFixed(2);
    }
    rec.winProb = rec.winProbByHorizon.positional;
  ...
  return { stocks, tilt: tilt.tiltLabel, riskScale: +riskScale.toFixed(3), macro: mac, calibrated: !!calibrate.calibrated, calibrationBasis: calibrate.basis || null };
}
