...
function mergeNamespace(ns, obj) {
  ...
  try {
    // Atomic write (tmp + rename): generators and the learner can run concurrently;
    // a torn write here would drop whole namespaces.
...
// Merge tuned signal-rule constants into `.signals` and persist (atomic).
// Used by optimize-signals.js after out-of-sample validation. `provenance` is an
// optional object recorded under `.signalsProvenance` for transparency. Respects
// the `frozen` flag (skips the write) so a human can lock the config.
function mergeSignals(obj, provenance) {
  let data;
  try {
    data = fs.existsSync(WEIGHTS_PATH) ? JSON.parse(fs.readFileSync(WEIGHTS_PATH, 'utf8')) : null;
  } catch { data = null; }
  if (!data || typeof data !== 'object') data = {};
  if (data.frozen) { console.warn('weights file is frozen — mergeSignals skipped'); return false; }
  data.signals = Object.assign({}, data.signals, obj);
  if (provenance) data.signalsProvenance = provenance;
  ...
  try {
    ...
    console.warn('weights mergeSignals write failed:', e.message);
    return false;
  }
  _loaded = false; // invalidate cache
  return true;
...
module.exports = {
  ...
  load, clampMult, getMult, getNamespace, getSignalTuning, mergeNamespace, mergeSignals,
};
