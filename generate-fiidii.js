...
function main() {
  ...
  const rows = agg.map(a => {
    ...
    const lp = livePrices[a.symbol]; // live feed, reconciled against the sidecar (rejects bad/split data)
    return {
      ...a, name,
      sector: b.sector || ax.sector || null,
      url: urls[a.symbol] || ax.url || null,
      price: reconcile((b.price ?? ax.price ?? null), lp && lp.p != null ? lp.p : null),
