...
(function(){
  ...
  function buildDrContent(s){
    ...
    html+='<div class="dr-section"><div class="dr-section-title">\ud83e\udde0 AI Deep Analysis <span style="font-size:.6rem;font-weight:400;text-transform:none;opacity:.6">(Google Gemini)</span></div>'
      ...
      +'<div style="margin-bottom:6px"><select id="dr-provider-select" onchange="drChangeProvider()" style="width:100%;background:var(--header-bg,#12121a);color:var(--t1,#e4e4ea);border:1px solid var(--card-border,#2a2a38);border-radius:6px;padding:7px 10px;font-size:.78rem;cursor:pointer">'
      +Object.keys(DR_PROVIDERS).map(function(k){return'<option value="'+k+'">'+DR_PROVIDERS[k].label+'</option>';}).join('')
      +'</select></div>'
      +'<div style="margin-bottom:6px"><select id="dr-model-select" style="width:100%;background:var(--header-bg,#12121a);color:var(--t1,#e4e4ea);border:1px solid var(--card-border,#2a2a38);border-radius:6px;padding:7px 10px;font-size:.78rem;cursor:pointer"></select></div>'
      ...
      +'<div style="font-size:.62rem;color:var(--t3,#5a5a70);margin-top:5px">Get free key at <a id="dr-key-link" href="https://console.groq.com/keys" target="_blank" rel="noopener" style="color:#00d4aa">console.groq.com</a> \u00b7 Stored only in your browser</div>'
      +'</div>';
    return html;
  }
